...
def test(args, split="test", model=None, tokenizer=None, test_dataset=None):
    ...
    f2 = open(args.output_path2, "w",encoding='utf-8')
    qrelf = open(split + '.' + args.qrels_path, "w", encoding='utf-8')
    ...
    if args.data_format == "trec":
        map, mrr, p30 = evaluate_trec(predictions_file=args.output_path2, qrels_file="qrels.microblog.txt") #\
